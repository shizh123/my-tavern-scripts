...
export const Schema = z.object({
  ...
  治疗: z
    .object({
      完成度: z.coerce.number().transform(v => Math.round(v * 10) / 10).transform(v => Math.max(0, Math.min(100, v))).prefault(0),
      // 由脚本根据完成度自动计算，AI不要修改
      阶段: z.coerce.number().transform(v => Math.max(1, Math.min(10, Math.floor(v)))).prefault(1),
    })
    .prefault({ 完成度: 0, 阶段: 1 }),
  ...
  云霜凝: z
    .object({
      信任度: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(5),
      心理防线: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(100),

      身体开发: z
        .object({
          小嘴: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(0),
          胸部: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(0),
          小屄: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(0),
          屁穴: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(0),
        })
      ...
      服装: z
        ...
        .prefault({ 上装: '寒霜门道袍', 下装: '寒霜门长裙', 内衣: '素白抹胸', 内裤: '素白亵裤', 特殊配饰: '无', 暴露程度: '遮蔽' }),
    ...
    .prefault({
      ...
      服装: { 上装: '寒霜门道袍', 下装: '寒霜门长裙', 内衣: '素白抹胸', 内裤: '素白亵裤', 特殊配饰: '无', 暴露程度: '遮蔽' },
      心理活动: '',
    }),
  ...
  苗广: z
    .object({
      疑心值: z.coerce.number().transform(v => Math.max(0, Math.min(100, v))).prefault(0),
      ...
      千晶幻术: z
        .object({
          ...
          冷却结束楼层: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
          已使用次数: z.coerce.number().transform(v => Math.max(0, Math.min(5, Math.floor(v)))).prefault(0),
  ...
  系统: z
    .object({
      灵石: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(100),
      // 道具三状态：未购买 / 已购买 / 使用中
  ...
  时间: z
    .object({
      第几天: z.coerce.number().transform(v => Math.max(1, Math.floor(v))).prefault(1),
      // 以小时为单位存储，如 14.5 = 14:30
      当前小时: z.coerce.number().transform(v => {
  ...
  _打断冻结至楼层: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
  ...
  // 净灵铃上次使用的天数（每天限用1次）
  _净灵铃上次使用楼层: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
  ...
  _临时道具剩余轮次: z.record(z.string(), z.coerce.number().transform(v => Math.max(0, Math.floor(v)))).prefault({}),
  // 特殊场景状态机（里程碑剧情多轮引导）
  _特殊场景: z
    .object({
      进行中: z.string().prefault(''), // 场景名，空串=无进行中场景
      当前阶段: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
      总阶段数: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
      当前阶段轮次: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0), // 当前阶段已经过的轮数
    })
  ...
  _记忆进入次数: z.record(z.string(), z.coerce.number().transform(v => Math.max(0, Math.floor(v)))).prefault({}),
  ...
  _留影石计数: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
  // 苗喧彩蛋：上次触发的楼层（频率控制，每10~15楼触发一次）
  _苗喧上次触发楼层: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
  // 苗喧碎片：当前显示在状态栏的叙事片段
  _苗喧碎片: z.string().prefault(''),
  // 千晶幻术当前施术开始的楼层（用于计算当前第几轮）
  _千晶幻术开始楼层: z.coerce.number().transform(v => Math.max(0, Math.floor(v))).prefault(0),
});
