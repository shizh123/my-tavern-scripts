...
interface UnlockCondition {
  ...
  需完成场景?: string[]; // 需要已完成的特殊场景 (AND 全部完成)
  需完成任一场景?: string[]; // 需要完成其中任一特殊场景 (OR 任一完成)
}
...
const ITEM_UNLOCK_CONDITIONS: Record<string, UnlockCondition> = {
  ...
  // ── 千晶幻术 ──
  // 2.0.41: 解锁顺序 — 寝取宣告 + (绿帽奴调教 OR 儿媳调教公公) → 千晶幻术
  //   两条路径二选一, 玩家做任一个即解锁; 千晶完成后仍可做另一个未完成的。
  千晶幻术: {
    阶段: 7,
    苗广心态: ['屈辱', '默许', '沉溺'],
    需完成场景: ['寝取宣告'],
    需完成任一场景: ['绿帽奴调教', '儿媳调教公公'],
  },
  ...
  // 2.0.41: 去掉"千晶幻术未完成"限制 — 千晶完成后仍可回头做原版
  绿帽奴调教: { 阶段: 7, 苗广心态: ['沉溺'], 需完成场景: ['寝取宣告'] },
...
export function canUnlockItem(itemName: string, data: SchemaType): boolean {
  ...
  if (cond.需完成任一场景 !== undefined) {
    const done = data._已完成特殊场景;
    if (!cond.需完成任一场景.some(s => !!done[s])) return false;
  }
