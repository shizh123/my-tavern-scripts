...
const ALL_ITEMS: Record<string, ItemDef[]> = {
  ...
  场景: [
    { name: '镜前调教', price: 400, type: '特殊场景', desc: '6阶段·面对镜子观察自己', unlockDesc: '阶段≥4，防线≤50' },
    {
      ...
      unlockDesc: '洛书晴线已激活，需先完成洛书晴现实初遇，洛≥3，云≥4',
    },
    {
      name: '夫前凌辱',
      price: 600,
...
function isUnlocked(item: ItemDef): boolean {
  ...
  const conds: Record<string, () => boolean> = {
    ...
    // ── 联动场景（洛书晴线已激活 + 现实初遇完成后） ──
    // 2.0.32: 加 `现实初遇完成` 前置——线激活只表示神魂空间认识过,
    //         联动场景需要两人在现实中正式见过(阶段 2→3 自动触发的 4 轮场景)。
    婆媳教导: () =>
      !!d._洛书晴线已激活 && !!d._已完成特殊场景['洛书晴现实初遇'] && d.洛书晴.调教阶段 >= 3 && d.治疗.阶段 >= 4,
    两人同侍: () =>
      !!d._洛书晴线已激活 &&
      !!d._已完成特殊场景['洛书晴现实初遇'] &&
      d.洛书晴.调教阶段 >= 5 &&
    ...
    双重改嫁: () =>
      ...
      d.苗广.千晶幻术.认知改写完成 &&
      !!d._已完成特殊场景['儿媳调教公公'],
    // 真相揭露场景: 千晶幻术解锁后但尚未使用的窗口期,单一一次性叙事
    未知: () => d.治疗.阶段 >= 7 && ['屈辱', '默许', '沉溺'].includes(d.苗广.心态) && d.苗广.千晶幻术.已使用次数 === 0,
    // 后日谈: 双重改嫁完成后解锁,单一前置
...
function meetsLuoUnlock(name: string): boolean {
  ...
  switch (name) {
    ...
    case '眼罩':
      return s >= 3 && c.心理防线 <= 70 && c.顺从度 >= 15;
    case '乳夹':
      return c.心理防线 <= 65 && c.身体开发.胸部 >= 40 && c.顺从度 >= 25;
    case '口枷':
      return c.心理防线 <= 65 && c.身体开发.小嘴 >= 40 && c.顺从度 >= 25;
    case '肛塞':
      return c.心理防线 <= 60 && c.身体开发.屁穴 >= 40 && c.顺从度 >= 20;
    case '缚灵缎':
      return c.心理防线 <= 60 && c.顺从度 >= 30;
    case '震动器':
      return c.心理防线 <= 50 && c.身体开发.小屄 >= 50 && c.顺从度 >= 35;
    // 体改(共用)
    case '丰胸灵乳丹·中':
      return s >= 3 && c.身体开发.胸部 >= 30;
    case '丰胸灵乳丹·大':
      return s >= 4 && c.身体开发.胸部 >= 50;
    case '丰胸灵乳丹·极':
      return s >= 6 && c.身体开发.胸部 >= 70;
    case '丰臀圆玉丹':
      return s >= 3 && c.身体开发.屁穴 >= 30;
    case '乳环':
      return s >= 4 && c.身体开发.胸部 >= 40;
    case '阴环':
      return s >= 4 && c.身体开发.小屄 >= 40;
    case '淫纹刻印':
      return s >= 4 && c.心理防线 <= 50;
    case '肉棒口罩':
      return s >= 5 && c.身体开发.小嘴 >= 40;
    // 性癖(共用 20 项)
    case '阿黑颜体质':
      return s >= 3 && c.心理防线 <= 50;
    case '潮喷体质':
      return s >= 4 && c.身体开发.小屄 >= 60;
    case '母乳体质':
      return s >= 4 && c.身体开发.胸部 >= 50;
    case '露出嗜好':
      return s >= 3 && c.心理防线 <= 50;
    ...
    case '哦齁齁体质':
      return s >= 5 && c.心理防线 <= 20;
    case '骚话淫语':
      return s >= 4 && c.心理防线 <= 40;
    case '隐奸行为': {
      const 苗广ok = ['察觉', '屈辱', '默许', '沉溺'].includes(d.苗广.心态);
      return 苗广ok && s >= 4 && c.心理防线 <= 40;
    }
    case '尿饮嗜好':
      return s >= 5 && c.心理防线 <= 20 && c.身体开发.小嘴 >= 40;
    case '母爱泛滥':
      return s >= 4 && c.心理防线 <= 30;
    case '舔肛嗜好':
      return s >= 4 && c.身体开发.屁穴 >= 40 && c.心理防线 <= 40;
    case '受虐嗜好':
      return s >= 4 && c.心理防线 <= 40;
    case '精液标记':
      return s >= 4 && c.心理防线 <= 30;
    case '口奴体质':
      return s >= 4 && c.身体开发.小嘴 >= 50 && c.心理防线 <= 30;
    case '肛交嗜好':
      return s >= 4 && c.身体开发.屁穴 >= 50 && c.心理防线 <= 40;
    case '物化认知':
      return s >= 6 && c.心理防线 <= 15;
    case '痴女化':
      return s >= 5 && c.心理防线 <= 25;
    case '身体书写':
      return s >= 4 && c.心理防线 <= 30;
    case '窒息快感':
