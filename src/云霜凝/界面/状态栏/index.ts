...
function checkScriptLoaded() {
  ...
  if (flag) return; // 脚本已加载，成功提示由脚本端 parent.toastr 处理

  const _top = (window.parent ?? window) as any;
  const ST = _top.SillyTavern;
  if (ST?.Popup && ST?.POPUP_TYPE) {
    const popup = new ST.Popup(
    ...
    _top.toastr?.error?.('游戏逻辑脚本未能正确加载，请检查酒馆助手配置', '云霜凝');
  }
}
