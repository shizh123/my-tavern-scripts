<template>
  <div class="yun-panel">
    ...
    <!-- 心理状态 -->
    <div class="card">
      <div class="section-title sec-mental"><span class="decor-line"></span><span>心理状态</span><span class="decor-line"></span></div>
      <div class="stat-list">
        <div class="stat-row">
    ...
    <!-- 身体开发 -->
    <div class="card">
      <div class="section-title sec-body"><span class="decor-line"></span><span>身体开发</span><span class="decor-line"></span></div>
      <div class="body-grid">
    ...
    <!-- 服装 -->
    <div class="card">
      <div class="section-title sec-attire"><span class="decor-line"></span><span>外观</span><span class="decor-line"></span></div>
    ...
    <template v-if="hasBodyMods">
      <div class="card">
        <div class="section-title sec-mod"><span class="decor-line"></span><span>肉体改造</span><span class="decor-line"></span></div>
        <div class="tag-grid">
    ...
    <template v-if="!_.isEmpty(store.data.云霜凝.性癖列表)">
      <div class="card">
        <div class="section-title sec-kink"><span class="decor-line"></span><span>性癖档案</span><span class="decor-line"></span></div>
        <div class="kink-wrap">
    ...
    <template v-if="activeSexToys.length > 0">
      <div class="card">
        <div class="section-title sec-toy"><span class="decor-line"></span><span>性具</span><span class="decor-line"></span></div>
        <div class="toy-wrap">
    ...
    <template v-if="activeItems.length > 0 || hasTempItems">
      <div class="card">
        <div class="section-title sec-item"><span class="decor-line"></span><span>当前道具</span><span class="decor-line"></span></div>
        <div class="active-items">
          <span v-for="name in activeItems" :key="name" class="active-tag">{{ name }}</span>
          <span
            v-for="(turns, name) in store.data._临时道具剩余轮次"
            :key="'tmp-' + name"
            class="active-tag temp-tag"
    ...
    <template v-if="store.data.云霜凝.心理活动">
      <div class="section-title sec-thought"><span class="decor-line"></span><span>心理活动</span><span class="decor-line"></span></div>
...
const exposeClass = computed(() => {
  const map: Record<string, string> = { 遮蔽: '', 微露: 'ex-min', 轻露: 'ex-low', 半露: 'ex-mid', 大露: 'ex-high', 极露: 'ex-max' };
  return map[store.data.云霜凝.服装.暴露程度] ?? '';
});
const hasBodyMods = computed(() => {
  const mod = store.data.云霜凝.肉体改造;
  return mod.胸部 !== '默认' || mod.臀部 !== '默认' || mod.乳环 || mod.阴环
    || mod.淫纹位置.length > 0 || mod.堕落烙印;
});

// 身体器具（性具）列表
const SEX_TOY_NAMES = new Set([
  '眼罩', '乳夹', '口枷', '肛塞', '缚灵缎', '震动器', '项圈', '肉棒口罩',
]);
...
const activeItems = computed(() =>
  Object.entries(store.data.系统.道具状态)
    .filter(([name, state]) => state === '使用中' && !SEX_TOY_NAMES.has(name) && !CLOTHING_SLOT[name] && !KINK_ITEM_MAP[name] && !CONSUMABLE_NAMES.has(name))
    .map(([name]) => name)
);
...
.yun-panel { display: flex; flex-direction: column; gap: 10px; }
...
.card {
  ...
  &:hover { border-color: rgba($c-pri, 0.25); }
}
...
// 区域标题颜色区分
.sec-mental { color: rgba($c-acc, 0.85);
  .decor-line { background: linear-gradient(90deg, transparent, rgba($c-acc, 0.3), transparent); }
}
.sec-body { color: rgba(#d09060, 0.9);
  .decor-line { background: linear-gradient(90deg, transparent, rgba(#d09060, 0.3), transparent); }
}
.sec-attire { color: rgba(#80b0d8, 0.9);
  .decor-line { background: linear-gradient(90deg, transparent, rgba(#80b0d8, 0.3), transparent); }
}
.sec-mod { color: rgba($c-danger, 0.8);
  .decor-line { background: linear-gradient(90deg, transparent, rgba($c-danger, 0.3), transparent); }
}
.sec-kink { color: rgba(#d060a0, 0.85);
  .decor-line { background: linear-gradient(90deg, transparent, rgba(#d060a0, 0.3), transparent); }
}
.sec-toy { color: rgba(#e04880, 0.85);
  .decor-line { background: linear-gradient(90deg, transparent, rgba(#e04880, 0.3), transparent); }
}
.sec-item { color: rgba($c-good, 0.85);
  .decor-line { background: linear-gradient(90deg, transparent, rgba($c-good, 0.3), transparent); }
}
.sec-thought { color: rgba($c-frost, 0.75);
  .decor-line { background: linear-gradient(90deg, transparent, rgba($c-frost, 0.25), transparent); }
}
...
.stat-lbl  {
  width: 60px; font-size: 0.88rem;
  flex-shrink: 0; letter-spacing: 0.5px;
  font-weight: 600;
}
.lbl-trust { color: rgba($c-good, 0.8); }
.lbl-defense { color: rgba($c-acc, 0.75); }
.stat-num  {
  width: 32px; text-align: right;
  font-size: 1rem; font-weight: 900;
  font-variant-numeric: tabular-nums;
}
.num-trust { color: lighten($c-good, 12%); text-shadow: 0 0 8px rgba($c-good, 0.2); }
.num-defense { color: $c-frost; text-shadow: 0 0 8px rgba($c-acc, 0.2); }
...
.body-item  {
  ...
  &:hover { background: rgba(#d09060, 0.06); }
}
.body-lbl   {
  width: 28px; font-size: 0.84rem; color: rgba(#d09060, 0.9);
  flex-shrink: 0; font-weight: 600;
}
.body-num   {
  width: 24px; text-align: right;
  font-size: 0.8rem; color: rgba(#d09060, 0.7);
  font-variant-numeric: tabular-nums;
}
.lv-badge {
  ...
  &.lv0 { color: rgba($c-sub, 0.45); }
  &.lv1 { color: $c-warn; background: rgba($c-warn, 0.1); border: 1px solid rgba($c-warn, 0.15); }
  &.lv2 { color: #d08030; background: rgba(#d08030, 0.1); border: 1px solid rgba(#d08030, 0.15); }
  &.lv3 { color: #d05040; background: rgba(#d05040, 0.1); border: 1px solid rgba(#d05040, 0.15);
    box-shadow: 0 0 4px rgba(#d05040, 0.1); }
  &.lv4 { color: $c-danger; background: rgba($c-danger, 0.12); border: 1px solid rgba($c-danger, 0.2);
    box-shadow: 0 0 6px rgba($c-danger, 0.15);
    animation: lvPulse 2s ease-in-out infinite;
  }
}
@keyframes lvPulse {
  0%, 100% { box-shadow: 0 0 6px rgba($c-danger, 0.15); }
  50% { box-shadow: 0 0 10px rgba($c-danger, 0.25); }
}

// ── 服装/外观 ──
.attire-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; }
.attire-item {
...
.a-slot {
  display: block; font-size: 0.68rem; color: rgba(#80b0d8, 0.65);
  margin-bottom: 4px; letter-spacing: 0.8px;
  text-transform: uppercase; font-weight: 600;
}
.a-val { color: #c8e0f8; font-weight: bold; font-size: 0.9rem; }

.ex-min  { color: rgba($c-warn, 0.6); }
.ex-low  { color: $c-warn; }
.ex-mid  { color: #d07030; font-weight: bold; text-shadow: 0 0 4px rgba(#d07030, 0.2); }
.ex-high { color: #d04040; font-weight: bold; text-shadow: 0 0 6px rgba(#d04040, 0.2); }
.ex-max  { color: $c-danger; font-weight: bold; text-shadow: 0 0 8px rgba($c-danger, 0.3);
  animation: exPulse 1.5s ease-in-out infinite; }
@keyframes exPulse {
  0%, 100% { text-shadow: 0 0 8px rgba($c-danger, 0.3); }
  50% { text-shadow: 0 0 14px rgba($c-danger, 0.5); }
}

// ── 当前生效道具 ──
.active-items { display: flex; flex-wrap: wrap; gap: 6px; }
.active-tag {
  ...
  &.temp-tag {
    ...
    .temp-turns { color: rgba($c-warn, 0.8); margin-left: 4px; font-size: 0.6rem; }
  }
}
...
.mod-label {
  color: rgba($c-danger, 0.7); font-weight: 600; font-size: 0.68rem;
  letter-spacing: 0.3px;
}
.mod-value {
  color: rgba(#e0a0c0, 0.9); font-weight: bold;
}

// ── 性具标签 ──
.toy-wrap { display: flex; flex-wrap: wrap; gap: 6px; }
.toy-empty { font-size: 0.82rem; color: rgba(#aaa, 0.5); font-style: italic; }
.toy-chip {
...
// ── 性癖标签 ──
.kink-wrap { display: flex; flex-wrap: wrap; gap: 6px; }
.kink-chip {
