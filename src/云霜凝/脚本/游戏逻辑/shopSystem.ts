...
export const INSTANT_EFFECTS: Record<string, (data: SchemaType) => void> = {
  ...
  蚀心露: data => {
    ...
    console.info(
      `[商店] 蚀心露：防线 -3 → ${data.云霜凝.心理防线}，信任 +3 → ${data.云霜凝.信任度}，疑心 +3 → ${data.苗广.疑心值}`,
    );

    // 隐藏效果：疑心值已进入察觉区间(50-69) + 隔音灵阵 + 未触发过 + 非特殊场景 → 屈辱跳转
    // 2.0.27 修 1: 原代码判 `心态 === '察觉'`,但心态字段由 stateValidation 重算,
    // shopSystem 在前,导致"疑心 49 → +3=52"时心态仍是旧值"疑惑",隐藏效果不触发。
    // 改用疑心值数值范围判定,与 calcMiaoguangMind 的察觉区间(50-69)一致。
    // 2.0.27 修 2: 特殊场景进行中禁用屈辱转换——强转变事件会和场景剧情严重冲突。
    // 显性效果(数值变化)照常,玩家可在场景结束后再次使用触发屈辱转换(前提仍在察觉区间)。
    const 进入察觉区间 = data.苗广.疑心值 >= 50 && data.苗广.疑心值 < 70;
    const 非特殊场景 = !data._特殊场景.进行中;
    if (进入察觉区间 && 非特殊场景 && data.系统.道具状态['隔音灵阵'] === '使用中' && !data._已触发蚀心露屈辱) {
      data._已触发蚀心露屈辱 = true;
      data.苗广.心态 = '屈辱';
  ...
  混沌珠: data => {
    ...
    const 心态 = data.苗广.心态;
    if (心态 === '屈辱' || 心态 === '默许' || 心态 === '沉溺' || 心态 === '愤怒') {
      console.warn(`[商店] 混沌珠：${心态}状态无效，不执行`);
      return;
    }
  ...
  // 洛书晴专属消耗品（仅阶段1-2 生效；阶段3+时 script 不再注入提示词）
  安抚符: data => {
    const floor = (globalThis as any).SillyTavern?.chat?.length ?? 0;
    if (data.洛书晴.调教阶段 >= 3) {
      console.warn('[商店] 安抚符在阶段3+不再生效，已静默消耗');
      return;
    }
    data._洛书晴_安抚符生效至 = floor + 3;
    console.info(`[商店] 安抚符：洛书晴敌意软化，生效至楼层${data._洛书晴_安抚符生效至}`);
  },
  真心符: data => {
    const floor = (globalThis as any).SillyTavern?.chat?.length ?? 0;
    if (data.洛书晴.调教阶段 >= 3) {
      console.warn('[商店] 真心符在阶段3+不再生效，已静默消耗');
      return;
    }
    data._洛书晴_真心符生效至 = floor + 3;
    console.info(`[商店] 真心符：洛书晴直接表达内心，生效至楼层${data._洛书晴_真心符生效至}`);
  },
};
...
const ALL_EQUIPMENT: Set<string> = new Set([
  ...
  '项圈',
  '肉棒口罩',
  // 环境类（原辅助灵物 → 移至环境类，类别变更后暖玉佩保留在装备集合中）
  '暖玉佩',
]);
...
/** 特殊配饰道具→子槽位映射（8部位） */
export const ACCESSORY_SUB_SLOT: Record<string, AccessorySubSlot> = {
  脚链铃铛: '脚踝',
  精液项链: '颈部',
  精液耳坠: '耳部',
  红绳: '腰部',
...
const BODY_MOD_EFFECTS: Record<string, (data: SchemaType) => void> = {
  '丰胸灵乳丹·中': data => {
    data.云霜凝.肉体改造.胸部 = 'E罩杯';
  },
  '丰胸灵乳丹·大': data => {
    data.云霜凝.肉体改造.胸部 = 'G罩杯';
  },
  '丰胸灵乳丹·极': data => {
    data.云霜凝.肉体改造.胸部 = 'H罩杯';
  },
  丰臀圆玉丹: data => {
    data.云霜凝.肉体改造.臀部 = '丰满';
  },
  乳环: data => {
    data.云霜凝.肉体改造.乳环 = true;
  },
  阴环: data => {
    data.云霜凝.肉体改造.阴环 = true;
  },
};
...
function applyEquipSideEffects(itemName: string, data: SchemaType): void {
  ...
  if (!slot) return;
  if (slot === '特殊配饰') {
    const sub = ACCESSORY_SUB_SLOT[itemName];
    if (sub) {
      data.云霜凝.服装.特殊配饰[sub] = itemName;
...
function applyUnequipSideEffects(itemName: string, data: SchemaType): void {
  ...
  if (slot === '特殊配饰') {
    const sub = ACCESSORY_SUB_SLOT[itemName];
    if (sub) {
      // 同子槽位是否有其他配饰在使用中（腰部/阴蒂互斥组）
      const otherInSub = Object.entries(ACCESSORY_SUB_SLOT).find(
        ([other, s]) => s === sub && other !== itemName && data.系统.道具状态[other] === '使用中',
      )?.[0];
      data.云霜凝.服装.特殊配饰[sub] = otherInSub ?? '';
      data.云霜凝.服装.暴露程度 = calcExposure(data);
    }
    return;
  }
  const groupName = slot as string;
  const otherInSlot = EXCLUSIVE_GROUPS[groupName]?.find(c => c !== itemName && data.系统.道具状态[c] === '使用中');
...
export function processNewlyActivatedItems(newData: SchemaType, oldData: SchemaType, currentFloor?: number): void {
  ...
  for (const [itemName, newState] of Object.entries(newItems)) {
    ...
    if (INSTANT_EFFECTS[itemName]) {
      ...
      if (cooldown && floor > 0) {
        ...
        if (lastUsedFloor > 0 && floor - lastUsedFloor < cooldown) {
          ...
          newData.系统.道具状态[itemName] = '已购买';
          console.warn(
            `[商店] ${itemName} 冷却中（上次楼层${lastUsedFloor}，当前${floor}，需间隔${cooldown}），已退回`,
          );
          continue;
        }
      }
    ...
    // ─── 淫纹刻印（特殊：需要位置+文字参数，格式 "淫纹刻印·<位置>·<文字>"）───
    ...
    if (KINK_ITEM_MAP[itemName]) {
      // 统计当前已激活的性癖数量
      const activeKinkCount = Object.keys(newData.系统.道具状态).filter(
        k => KINK_ITEM_MAP[k] && newData.系统.道具状态[k] === '使用中',
      ).length;
    ...
    if (ALL_EQUIPMENT.has(itemName)) {
      ...
      // 锚神钉全局唯一：装备给云霜凝时，若洛书晴的也是"使用中"则自动卸下
      if (itemName === '锚神钉' && newData._洛书晴道具状态['锚神钉'] === '使用中') {
        newData._洛书晴道具状态['锚神钉'] = '已购买';
        console.info('[商店] 锚神钉全局唯一：从洛书晴自动卸下');
      }
...
export function processNewlyActivatedLuoItems(newData: SchemaType, oldData: SchemaType, currentFloor?: number): void {
  ...
  for (const [itemName, newState] of Object.entries(newItems)) {
    ...
    if (
      newData.洛书晴.调教阶段 < 3 &&
      (CLOTHING_SLOT[itemName] || BODY_MOD_EFFECTS[itemName] || KINK_ITEM_MAP[itemName])
    ) {
      newData._洛书晴道具状态[itemName] = '已购买';
      console.warn(`[商店] 洛书晴阶段<3，${itemName} 无法激活`);
      continue;
    }

    // ─── 洛书晴消耗品（阶段3+ 时脚本不再注入提示词，但仍执行 INSTANT_EFFECTS）
    if (INSTANT_EFFECTS[itemName]) {
      const cooldown = CONSUMABLE_COOLDOWN[itemName];
      const floor = currentFloor ?? 0;
      if (cooldown && floor > 0) {
        const lastUsedKey = `洛书晴_${itemName}`;
        const lastUsedFloor = newData._消耗品上次使用楼层[lastUsedKey] ?? 0;
        if (lastUsedFloor > 0 && floor - lastUsedFloor < cooldown) {
          newData._洛书晴道具状态[itemName] = '已购买';
          console.warn(`[商店] 洛书晴 ${itemName} 冷却中，已退回`);
          continue;
        }
    ...
    // ─── 洛书晴体改（复用 BODY_MOD_EFFECTS，但写入洛书晴.肉体改造）
    if (BODY_MOD_EFFECTS[itemName]) {
      // 临时重定向：把 data.云霜凝 指向 data.洛书晴 完成体改后恢复
      // 简化：直接在此处手写共用逻辑（避免 BODY_MOD_EFFECTS 硬绑云霜凝）
      applyBodyModToLuo(itemName, newData);
      delete newData._洛书晴道具状态[itemName];
      console.info(`[商店] 洛书晴 ${itemName} 体改已写入`);
      continue;
    }
    ...
    // ─── 性癖道具（最多3个使用中，洛书晴独立槽位）
    if (KINK_ITEM_MAP[itemName]) {
      const activeKinkCount = Object.keys(newData._洛书晴道具状态).filter(
        k => KINK_ITEM_MAP[k] && newData._洛书晴道具状态[k] === '使用中',
      ).length;
      if (activeKinkCount > 3) {
        newData._洛书晴道具状态[itemName] = '已购买';
    ...
    // ─── 服装：写入 洛书晴.服装（无暴露等级计算）
    const slot = CLOTHING_SLOT[itemName];
    if (slot) {
      if (slot === '特殊配饰') {
        const sub = ACCESSORY_SUB_SLOT[itemName];
        if (sub) newData.洛书晴.服装.特殊配饰[sub] = itemName;
      } else {
        newData.洛书晴.服装[slot] = itemName;
      }
      continue;
    }

    // ─── 身体器具（保持"使用中"即可，注入时从状态表读取）
    if (ALL_EQUIPMENT.has(itemName)) {
      // 锚神钉全局唯一：装备给洛书晴时，云霜凝的自动卸下
      if (itemName === '锚神钉' && newData.系统.道具状态['锚神钉'] === '使用中') {
        newData.系统.道具状态['锚神钉'] = '已购买';
        console.info('[商店] 锚神钉全局唯一：从云霜凝自动卸下');
      }
      continue;
    }
  }
...
/** 洛书晴体改写入（简化版，复用云霜凝体改的文本但写入洛书晴） */
function applyBodyModToLuo(itemName: string, data: SchemaType): void {
  switch (itemName) {
    case '丰胸灵乳丹·中':
      data.洛书晴.肉体改造.胸部 = 'E罩杯';
      break;
    case '丰胸灵乳丹·大':
      data.洛书晴.肉体改造.胸部 = 'G罩杯';
      break;
    case '丰胸灵乳丹·极':
      data.洛书晴.肉体改造.胸部 = 'H罩杯';
      break;
    case '丰臀圆玉丹':
      data.洛书晴.肉体改造.臀部 = '丰满';
      break;
    case '乳环':
      data.洛书晴.肉体改造.乳环 = true;
      break;
    case '阴环':
      data.洛书晴.肉体改造.阴环 = true;
      break;
  }
}
...
export function applySpecialSceneConsequences(scene: string, data: SchemaType): void {
  switch (scene) {
    ...
    case '婆媳教导':
      ...
      data.苗喧.绝望值 = Math.min(100, data.苗喧.绝望值 + 3);
      data.苗喧.压抑值 = Math.min(100, data.苗喧.压抑值 + 8);
      data.云霜凝.信任度 = Math.min(100, data.云霜凝.信任度 + 3);
      break;
    ...
    case '双重目击':
      data.洛书晴.心理防线 = Math.max(0, data.洛书晴.心理防线 - 2);
...
export function syncClothingFromState(data: SchemaType): void {
  ...
  for (const [itemName, state] of Object.entries(data.系统.道具状态)) {
    ...
    if (slot === '特殊配饰') {
      const sub = ACCESSORY_SUB_SLOT[itemName];
      if (sub && data.云霜凝.服装.特殊配饰[sub] !== itemName) {
        data.云霜凝.服装.特殊配饰[sub] = itemName;
        changed = true;
        console.info(`[商店] 服装同步修复: 特殊配饰.${sub} → ${itemName}`);
      }
      continue;
    }
    if (data.云霜凝.服装[slot] !== itemName) {
      data.云霜凝.服装[slot] = itemName;
      changed = true;
